...
- **Taille d'upload côté NPM (gotcha 413)** : Nginx Proxy Manager applique le défaut nginx `client_max_body_size 1m` → tout upload > 1 Mo est coupé par le proxy avec « 413 Request Entity Too Large » **avant** d'atteindre le conteneur (l'app, elle, renvoie un JSON et autorise `MAX_UPLOAD_MB`). Correctif : Proxy Host → Edit → onglet **Advanced** → coller le snippet `deploy/npm-advanced.conf` (`client_max_body_size 0;` = délègue la limite à l'app). Ce n'est PAS un correctif de code.
